...
namespace UnityStandardAssets.Characters.FirstPerson
{
    ...
    public class FirstPersonController : MonoBehaviour, IMovement
    {
        ...
        private void Start()
        {
            m_Attack = GetComponent<AttackController>();
        ...
        private void Swim()
        {
            ...
            // ---- MOVE UPWARDS ----
            m_MoveDir.y = 0;
            var gravityOffset = new Vector3(0, 0.1f, 0);

            if (m_Jump)
            {
                if (Singleton_Water.Instance.SwimmingCheck(m_Camera.transform.position + gravityOffset)) { m_MoveDir.y = m_SwimSpeed; }
                else { m_MoveDir.y = 0f; }                
            }
            else if (!m_IsWalking)
            {
            ...
            {                
                if (Singleton_Water.Instance.SwimmingCheck(m_Camera.transform.position + gravityOffset)) { m_MoveDir.y = 0.5f; }
                else { m_MoveDir.y = 0f; }
            }
        ...
        private void GetInput(out float speed)
        {
            ...
            if (m_Disabled) { m_Input = new Vector2(0, 0); }
